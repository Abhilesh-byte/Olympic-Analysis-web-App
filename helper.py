...
def medal_tally(df):
    ...
    medal_tally = medal_tally.groupby('region').sum(numeric_only=True)[
        ['Gold', 'Silver', 'Bronze']
...
def fetch_medal_tally(df, year, country):
    ...
    if flag == 1:
        ...
        x = temp_df.groupby('region').sum(numeric_only=True)[
            ['Gold', 'Silver', 'Bronze']
        ].sort_values('Gold', ascending=False).reset_index()
...
def most_successful(df, sport):
    ...
    x = (
        ...
        .reset_index()
        .head(15)
        .merge(df, on='Name', how='left')[['Name', 'count', 'Sport', 'region']]
...
def most_successful_countrywise(df, country):
    ...
    x = (
        ...
        .reset_index()
        .head(10)
        .merge(df, on='Name', how='left')[['Name', 'count', 'Sport']]
        .drop_duplicates('Name')
    )
    x.rename(columns={'count': 'Medals'}, inplace=True)
    return x
